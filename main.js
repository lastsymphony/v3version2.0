...
global.owner = ['6289670394574'] // Put your number here
  ...
  try {
    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        let isROwner = [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '6289670394574@s.whatsapp.net').includes(m.sender)
        ...
        m.plugin = name
        let isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '6289670394574@s.whatsapp.net').includes(m.sender)
        let isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '') + '6289670394574@s.whatsapp.net').includes(m.sender)
...
if (opts['test']) {
  conn.user = {
    jid: '6289670394574@s.whatsapp.net',
    name: 'last',
    phone: {6289670394574}
  }
  conn.chats
  ...
  rl.on('line', line => conn.sendMessage('6289670394574@s.whatsapp.net', line.trim(), 'conversation'))
} else {
  rl.on('line', line => {
    global.DATABASE.save()
